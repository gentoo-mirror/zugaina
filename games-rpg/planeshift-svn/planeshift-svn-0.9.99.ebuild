...
# $Header: /var/cvsroot/gentoo-x86/games-rpg/planeshift-svn/planeshift-svn-0.9.99.ebuild,v 1.3 2008/08/18 12:00:00 loux.thefuture Exp $

inherit subversion eutils games flag-o-matic

ESVN_REPO_URI="https://planeshift.svn.sourceforge.net/svnroot/planeshift/stable"
ESVN_PROJECT="planeshift"
...
src_install() {
	...
	buildShFile psupdater
	buildShFile pssetup
	buildShFile psclient
...
buildShFile() {
	file=$1
	einfo "Creation de ${file}.sh"
	cat > "${file}.sh" << EOF
unset CRYSTAL_CONFIG
unset CRYSTAL_PLUGIN
CS_VERSION=`/opt/planeshift/crystalspace/bin/cs-config --version`
export CRYSTAL="/opt/planeshift/crystalspace/lib/crystalspace-${CS_VERSION}"
export LD_LIBRARY_PATH=/opt/planeshift/crystalspace/lib
...
buildUpdaterInfo() {
	file="updaterinfo.xml"
	einfo "Building ${file}"
	cat > "${file}" << EOF
<config>
    <mirrors>
        <mirror id="1" name="psmirror" url="http://www.psmirror.org/repo/" />
        <mirror id="2" name="xordan" url="http://www.xordan.com/psupdater/" />
    </mirrors>
</config>
EOF
}
...
MY_PV="viewer-rc-frozen-${PV/_rc/}.122255"
MY_DATE="2009/05"
GLH_V="20080812"
SDL_V="20080818"
...
SRC_URI="http://automated-builds-secondlife-com.s3.amazonaws.com/viewer-rc-frozen/slviewer-src-${MY_PV}.tar.gz
	http://automated-builds-secondlife-com.s3.amazonaws.com/viewer-rc-frozen/slviewer-artwork-${MY_PV}.zip
	http://automated-builds-secondlife-com.s3.amazonaws.com/viewer-rc-frozen/slviewer-linux-libs-${MY_PV}.tar.gz
...
src_unpack() {
	...
	use vivox && unpack vivox-linux-${VIVOX_V}.tar.bz2
	cd "${S}"
	rm -rf linden/libraries
	rm -rf linden/indra/newview/app_settings

	epatch "${FILESDIR}"/"${PV}"/${PN}-cmake.patch
	epatch "${FILESDIR}"/"${PV}"/VWR-10579_make-cmake-see-library-mkII.patch
	epatch "${FILESDIR}"/"${PV}"/secondlife-llmozlib-svn.patch
	epatch "${FILESDIR}"/"${PV}"/secondlife-llvoiceclient.patch
	epatch "${FILESDIR}"/"${PV}"/VWR-12995.patch
...
src_compile() {
	...
	mycmakeargs="${mycmakeargs} -DNDOF_FIND_REQUIRED:BOOL=FALSE"

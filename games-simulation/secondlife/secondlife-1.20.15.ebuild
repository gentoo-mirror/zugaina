...
SRC_URI="http://secondlife.com/developers/opensource/downloads/${MY_DATE}/slviewer-src-${MY_PV}.tar.gz
	...
	http://download-secondlife-com.s3.amazonaws.com/SecondLife_i686_${MY_BIN_PV}.tar.bz2
...
#	fi
	...
	epatch ${FILESDIR}/${PV}/${PN}-llmediamanager.patch
	epatch ${FILESDIR}/${PV}/${PN}-llversionviewer.patch
	if use llmozlib ; then
	    epatch ${FILESDIR}/${PV}/${PN}-llmozlib-svn.patch
	    epatch ${FILESDIR}/${PV}/${PN}-llviewermedia.patch
	fi
	if use openal ; then
...
src_install() {
	...
	doins -r app_settings character fonts skins || die
	cp -r "${S}"/indra/newview/res/* ${D}/${dir}/res-sdl || die
	# Not all cursors are in artwork package...
	cp -r "${WORKDIR}"/SecondLife_i686_${MY_BIN_PV}/res-sdl/* ${D}/${dir}/res-sdl
	...
	doexe linux_tools/handle_secondlifeprotocol.sh || die
	doexe linux_tools/register_secondlifeprotocol.sh || die
	...
	if use llmozlib ; then
	    ...
	    ln -s "/usr/$(get_libdir)/llmozlib-svn/runtime_release" "${D}/${dir}/app_settings/mozilla"
	    #dosym /usr/lib/llmozlib-svn/runtime_release "${dir}"/app_settings/mozilla-runtime-linux-`uname -m`

...
MY_PV="Branch_1-20-Viewer-r84760"
MY_DATE="2008/04"
...
src_unpack() {
	...
	epatch ${FILESDIR}/${P}-faster_wind-7.patch
	if use openal ; then
	    epatch ${FILESDIR}/${P}-openal.patch
	    epatch ${FILESDIR}/${P}-gstreamer_audio.patch
	fi
	sed -i -e "s/gcc_bin = .*$/gcc_bin = '$(tc-getCXX)'/" SConstruct || die
	sed -i -e "s/#define LL_DEBUG_GL 1/#define LL_DEBUG_GL 0/" llwindows/llgl.cpp
}

src_compile() {
	local myarch
	local myopts="BUILD=release STANDALONE=yes BTARGET=client DISTCC=no MOZLIB2=no GRID=agni"
	if [ "${ARCH}" == "x86" ] ; then

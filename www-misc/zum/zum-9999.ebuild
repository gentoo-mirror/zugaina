...
DESCRIPTION="A fork of ZoneMinder allows you to capture, analyse, record and monitor any cameras attached to your system."
HOMEPAGE="http://www.zoneminder.com/"
ESVN_REPO_URI="https://svn.unixmedia.net/public/zum/trunk/zum/"
ESVN_PROJECT="${PN}"
LICENSE="GPL-2"
KEYWORDS=""
IUSE="debug ffmpeg mmap haar anpr"
# plugins"
SLOT="0"
...
# It installs fine via g-cpan, but there's no ebuild currently in portage.
        ...
        if use haar; then
                cd ${S}/plugins/libzm_plugin_face_haar
                escons
        fi
        if use anpr; then
                cd ${S}/plugins/libzm_plugin_anpr
                make
        fi
}
...
src_install() {
        ...
        insinto /usr/share/zoneminder/db
        ...
        if use anpr; then
            insinto /usr/share/zm
            doins plugins/libzm_plugin_anpr/libzm_plugin_anpr.zmpl
            doins plugins/libzm_plugin_anpr/plugin.conf
            insinto /var/www/zoneminder/htdocs/skins/classic/views/css
            doins web/skins/classic/views/css/plugin.css
            insinto /var/www/zoneminder/htdocs/skins/classic/views/js
            doins web/skins/classic/views/js/plugin.js
            insinto /var/www/zoneminder/htdocs/skins/classic/views
            doins web/skins/classic/views/plugin.php
